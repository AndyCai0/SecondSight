...
final class AssemblyAIStreamingService: NSObject, AudioRenderer, @unchecked Sendable {
    ...
    func stop() {
        // A user-visible OFF state is a hard boundary: once this method
        // returns, no renderer callback can enqueue more audio and the socket
        // has been cancelled. The serial queue also waits for an in-flight
        // render conversion to finish before teardown begins.
        queue.sync { stopImmediately() }
    }

    func render(pcmBuffer: AVAudioPCMBuffer) {
        // LiveKit owns the callback buffer. Consume/convert it before render()
        // returns instead of retaining it across an asynchronous boundary.
        queue.sync { accept(pcmBuffer) }
    }
